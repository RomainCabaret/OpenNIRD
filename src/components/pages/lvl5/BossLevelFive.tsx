...
const HackerBuddy = ({
  ...
  return (
    <div className="hidden md:flex flex-col items-center justify-center gap-4 w-40 p-4 border-l border-green-900/30 bg-slate-900/40 backdrop-blur-sm h-[80vh] mt-12 transition-all flex-shrink-0">
      <div className="text-[10px] font-mono text-green-700 mb-2 tracking-widest text-center">
        ASSISTANT IA
      </div>
      ...
      <div className="flex gap-1 items-end h-12 mt-6 opacity-50">
        {[...Array(4)].map((_, i) => (
          <div
            key={i}
            className={`w-1.5 bg-green-500 transition-all duration-100`}
            style={{
...
export default function PianoTalesGame() {
  ...
  const spawnNote = useCallback((timestamp: number, difficulty: number) => {
    ...
    if (timestamp - lastSpawnTime.current >= currentSpawnRate) {
      ...
      for (const lane of availableLanes) {
        ...
        const lastNote = notesInLane.reduce((prev, curr) =>
    ...
    (timestamp: number) => {
      ...
      // Mode Secours
        ...
        currentProgress = Math.min(
          100,
          (timeElapsed / FALLBACK_DURATION_MS) * 100
        );
      ...
      notesRef.current = notesRef.current.filter(
        (note) => !note.processed && note.y < 120
      );
  ...
  const startGame = async () => {
    if (!audioSrc) {
      setAudioError("Aucune source audio.");
      return;
    }
    ...
    if (audioRef.current) {
      try {
        ...
        setAudioError("Lecture impossible (CORS ou format).");
      }
    }
  };
    ...
    <div className="flex flex-col items-center justify-center min-h-screen bg-slate-950 text-green-500 font-mono select-none overflow-hidden touch-none">
          ...
          <div className="flex justify-between items-end border-b border-green-800/30 pb-4">
            <div className="flex flex-col">
              ...
              <div className="flex items-center gap-2 text-xs text-green-700 font-mono mt-1">
                <Disc
                  size={12}
                  className={gameState === "PLAYING" ? "animate-spin" : ""}
                />
                <span className="truncate max-w-[150px]">{trackName}</span>
                {difficultyLevel > 1 && (
                  <span className="text-green-300 ml-2">
                    Surcadençage: x{difficultyLevel.toFixed(2)}
                  </span>
                )}
              </div>
            </div>
      ...
      <div className="flex flex-row items-center gap-8 z-10 w-full justify-center">
        ...
        <div className="relative w-full max-w-md h-[80vh] mt-12 transition-all duration-100 flex-shrink-0">
          ...
          <div
            className={`absolute -left-3 md:-left-6 top-0 bottom-0 w-2 md:w-3 bg-slate-900 border border-green-900/50 rounded-full overflow-hidden transition-all duration-500 ${
              milestoneTrigger
                ? "shadow-[0_0_30px_#fff] border-white scale-110"
                : ""
            }`}
          >
            <div
          ...
          <div
            className={`absolute -right-3 md:-right-6 top-0 bottom-0 w-2 md:w-3 bg-slate-900 border border-green-900/50 rounded-full overflow-hidden transition-all duration-500 ${
              milestoneTrigger
                ? "shadow-[0_0_30px_#fff] border-white scale-110"
                : ""
            }`}
          ...
          <div
            className={`relative w-full h-full flex border-x border-green-900/30 bg-slate-900/80 backdrop-blur-sm shadow-[0_0_20px_rgba(0,0,0,0.8)] overflow-hidden box-border ${
              milestoneTrigger
                ? "border-white/50 bg-slate-800/80 shadow-[0_0_50px_rgba(255,255,255,0.2)]"
                : ""
          ...
          >
            ...
            <div
              className="absolute w-full z-0 pointer-events-none border-t border-green-500/50 bg-green-500/5 shadow-[0_0_15px_rgba(34,197,94,0.2)]"
              style={{
                top: `${HIT_ZONE_TOP}%`,
                height: `${HIT_ZONE_BOTTOM - HIT_ZONE_TOP}%`,
              }}
            />

            {[0, 1, 2].map((laneIndex) => {
              const activeLongNote = notes.find(
                (n) =>
                  n.lane === laneIndex &&
                  n.type === "long" &&
                  !n.missed &&
              ...
              const incomingLongNote = notes.find(
                (n) =>
                  n.lane === laneIndex &&
                  n.type === "long" &&
                  !n.missed &&
              ...
              return (
                <div
                  key={laneIndex}
                  className={`relative flex-1 h-full z-10 transition-all duration-100 ease-out ${
                    activeLanes[laneIndex]
                      ? "bg-green-900/20 shadow-[inset_0_0_20px_rgba(34,197,94,0.1)]"
                      : ""
                  }`}
                >
                  {notes
                    ...
                    .map((note) => {
                      ...
                      const showCircle =
                        note.type === "long" && !note.missed && !note.processed;

                      return (
                        <React.Fragment key={note.id}>
                          {/* Notes ALIGNÉES avec les boutons */}
                          <div
                            className={`absolute w-3/4 left-0 right-0 mx-auto rounded-sm border-2 transition-opacity duration-75 flex flex-col justify-center overflow-hidden ${
                              note.missed
                                ? "border-red-500 bg-red-900/50 shadow-[0_0_20px_red]"
                                : "border-green-400 bg-green-900/40 shadow-[0_0_15px_#4ade80]"
                            } `}
                            style={{
                              top: `${note.y}%`,
                              height: `${note.length}%`,
                              opacity:
                                note.processed && !note.isHeld
                                  ? 0
                                  : note.missed
                                  ? 0.5
                                  : 1,
                          ...
                          >
                            {!note.missed && (
                              <div
                                ...
                                style={{
                                  backgroundImage: `repeating-linear-gradient(0deg, transparent, transparent 2px, rgba(34, 197, 94, 0.3) 3px)`,
                                }}
                              ></div>
                            )}
                          ...
                          {showCircle && (
                            <div
                              className={`absolute left-1/2 -translate-x-1/2 w-6 h-6 rounded-full border-2 flex items-center justify-center z-30 transition-transform duration-75 ${
                                note.isHeld
                                  ? "border-white bg-green-400 shadow-[0_0_20px_#4ade80] scale-110"
                                  : "border-green-300 bg-green-900 shadow-[0_0_10px_#22c55e]"
                              }`}
                              style={{ top: `${circleY}%`, marginTop: "-12px" }}
                            >
                              <div
                                className={`w-1.5 h-1.5 bg-white rounded-full ${
                                  note.isHeld ? "animate-ping" : ""
                                }`}
                              ></div>
                            </div>
                          )}
                  ...
                  <div
                    className={`absolute w-3/4 left-0 right-0 mx-auto h-12 border-2 flex items-center justify-center transition-all duration-75 relative ${
                      activeLanes[laneIndex]
                        ? "border-green-400 bg-green-500/20 shadow-[0_0_15px_#22c55e] translate-y-1"
                        : "border-green-800 bg-slate-900/60 hover:border-green-600"
                    }`}
                  ...
                  >
                    ...
                    {showHoldCircle && (
                      <div
                        className={`absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-8 h-8 rounded-full border-2 border-green-400 flex items-center justify-center ${
                          activeLanes[laneIndex]
                            ? "scale-110 shadow-[0_0_15px_#4ade80] bg-green-500/30"
                            : "opacity-50 animate-pulse"
                        }`}
                      >
                        <div
                          className={`w-2 h-2 bg-green-300 rounded-full ${
                            activeLanes[laneIndex] ? "animate-ping" : ""
                          }`}
                        ></div>
                      </div>
                    )}
        ...
        <div className="absolute inset-0 z-50 flex items-center justify-center bg-slate-950/95 p-6 font-mono">
          <div className="w-full max-w-sm space-y-8 text-center animate-in fade-in zoom-in duration-500 border border-green-900 p-8 rounded shadow-[0_0_50px_rgba(0,255,0,0.1)]">
            <div className="space-y-2">
              <h1 className="text-4xl font-black text-green-500 tracking-tighter drop-shadow-[0_0_10px_#22c55e]">
                INTRUSION SYSTÈME
              </h1>
              <p className="text-green-800 text-xs tracking-[0.5em] uppercase">
                V 6.3 // NO LAG
              </p>
            </div>
            ...
            <div className="space-y-4 pt-4 text-left">
              <div className="bg-slate-900/50 p-4 border border-green-800">
                <p className="text-[10px] text-green-600 mb-2 uppercase">
                  Flux Audio Cible
                </p>
              ...
              <button
                onClick={startGame}
                disabled={!isAudioLoaded && !audioSrc}
                className={`w-full py-4 border font-bold tracking-widest uppercase transition-all ${
                  isAudioLoaded || audioSrc
                    ? "border-green-500 bg-green-900/20 text-green-400 hover:bg-green-500 hover:text-black hover:shadow-[0_0_20px_#22c55e]"
              ...
              >
                <div className="flex items-center justify-center gap-3">
                  ...
                  <span>
                    {isAudioLoaded ? "EXÉCUTER" : "INITIALISATION..."}
                  </span>
                </div>
              </button>
            </div>
            {audioError && (
              <div className="text-red-500 text-xs bg-red-900/10 p-2 border border-red-900 flex items-center justify-center gap-2">
                <AlertCircle size={14} /> <span>{audioError}</span>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="absolute inset-0 z-50 flex items-center justify-center bg-slate-950/95 text-green-500 font-mono">
          <div className="text-center space-y-8 animate-in slide-in-from-bottom-10 p-8 border border-green-500 bg-slate-900 shadow-[0_0_50px_rgba(34,197,94,0.2)] max-w-sm w-full">
            <div>
              ...
              >
                {gameState === "VICTORY" ? "ACCÈS AUTORISÉ" : "ÉCHEC SYSTÈME"}
              </h2>
              <div className="h-px w-full mt-4 bg-green-900"></div>
            </div>
            <div className="py-4">
              <p className="text-green-800 text-xs uppercase tracking-widest mb-2">
                Données Récupérées
              </p>
              <p className="text-6xl text-white drop-shadow-[0_0_15px_rgba(255,255,255,0.5)]">
                {score}
              </p>
            </div>
            <button
              ...
              className="w-full flex items-center justify-center gap-2 px-6 py-4 bg-green-900/20 border border-green-500 text-green-400 hover:bg-green-500 hover:text-black font-bold uppercase tracking-wider transition-all"
            >

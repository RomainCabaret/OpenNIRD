...
const INITIAL_UPGRADES: Upgrade[] = [
  { id: 'adblock', name: 'AdBlocker v1', baseCost: 10, currentCost: 10, cps: 1, count: 0, icon: '🛡️', desc: 'Bloque les pubs basiques' },
  { id: 'proxy', name: 'VPN Crypté', baseCost: 62, currentCost: 62, cps: 10, count: 0, icon: '🌐', desc: 'Tunnel sécurisé rapide' },
  { id: 'server', name: 'Botnet Global', baseCost: 312, currentCost: 312, cps: 60, count: 0, icon: '👾', desc: 'Armée de machines zombies' },
  { id: 'ai', name: 'I.A. Quantique', baseCost: 1875, currentCost: 1875, cps: 300, count: 0, icon: '🧠', desc: 'Conscience artificielle' },
  { id: 'freedom', name: 'INTERNET LIBRE', baseCost: 12500, currentCost: 12500, cps: 0, count: 0, icon: '🕊️', desc: 'La fin de la surveillance', isFinal: true }
];
...
export default function CookieDestroyer() {
  ...
  // Game Phase
  const [gameState, setGameState] = useState<'PLAYING' | 'BOSS' | 'VICTORY'>('PLAYING');
  ...
  useEffect(() => {
    ...
    const loop = setInterval(() => {
      ...
      if (cps > 5 && !activePopup && Math.random() < 0.005) {
        ...
        setActivePopup({ id: Date.now(), x: Math.max(0, x), y: Math.max(0, y) });
  ...
  // --- ACTIONS ---
  const spawnParticle = useCallback((x: number, y: number, text: string, isGold = false) => {
  ...
  const handleBossClick = (id: number) => {
    ...
    spawnParticle(window.innerWidth / 2, window.innerHeight / 2, `${newRemaining} RESTANTS`, true);
    if (newRemaining <= 0) {
  ...
  // --- RENDER ---
  return (
    <div className={`relative w-full h-screen overflow-hidden bg-[#050507] text-[#f3f4f6] font-mono flex flex-col ${shake ? 'animate-shake' : ''}`}>
      ...
      {activePopup && gameState === 'PLAYING' && (
        <div 
          ...
          style={{ left: activePopup.x, top: activePopup.y, fontFamily: 'Arial, sans-serif' }}
        >
        ...
        <div className="relative z-10 grid grid-cols-1 grid-rows-[minmax(0,1fr)_minmax(0,1fr)] md:grid-rows-1 md:grid-cols-[1fr_320px] lg:grid-cols-[1fr_400px] h-full w-full">
          ...
          <div className="flex flex-col p-2 md:p-5 relative overflow-hidden items-center justify-between">
            ...
            <div className="flex-1 flex items-center justify-center relative w-full">
              ...
              >
                <span className="relative z-10 text-center">REFUSER<br/>COOKIES</span>
            ...
            <div className="h-[100px] md:h-[180px] w-full bg-black/60 backdrop-blur-sm border-t-2 border-violet-500 p-2 md:p-4 flex flex-col mt-2">
              <div className="text-gray-400 text-[10px] md:text-xs uppercase mb-1 md:mb-2">Réseau Actif (Unités)</div>
              <div className="flex flex-wrap content-start gap-1 md:gap-2 overflow-y-auto pr-1 scrollbar-thin scrollbar-thumb-violet-900 scrollbar-track-transparent h-full">
                {activeUnits.map((icon, idx) => (
                  <div key={idx} className="w-8 h-8 md:w-10 md:h-10 bg-white/5 border border-violet-500/60 rounded md:rounded-lg flex items-center justify-center text-lg md:text-2xl relative animate-[popIn_0.3s_ease]">
                    {icon}
          ...
          <div className="bg-[#111116]/95 backdrop-blur-md border-t md:border-t-0 md:border-l border-violet-500/60 flex flex-col h-full overflow-hidden">
            <div className="p-3 md:p-5 bg-gradient-to-r from-transparent via-violet-500/10 to-transparent border-b border-gray-800 text-center shrink-0">
              <h2 className="text-lg md:text-xl font-bold tracking-widest">BLACK MARKET</h2>
            </div>
              ...
              {upgrades.map((u) => {
                ...
                return (
                  <div 
                    ...
                    `}
                  >
                    <div className="text-2xl md:text-3xl flex items-center justify-center">{u.icon}</div>
                    <div>
                      <h4 className="font-bold text-white text-sm md:text-base">
                        {u.name} <span className="text-[10px] md:text-xs opacity-70 ml-1">x{u.count}</span>
                      </h4>
                      <div className="text-amber-400 font-mono font-bold text-xs md:text-sm">
                        {u.currentCost.toLocaleString()}
                      </div>
                      <div className="text-[10px] md:text-xs text-gray-400 mt-0.5 leading-tight">{u.desc}</div>
                    </div>
                  </div>
                );
              })}
      ...
      {/* VICTORY SCREEN */}
      {gameState === 'VICTORY' && (
        <div className="fixed inset-0 bg-black z-[60] flex flex-col items-center justify-center animate-[fadeIn_2s] text-center p-4">
          <h1 className="text-4xl md:text-6xl font-black text-transparent bg-clip-text bg-gradient-to-r from-white to-violet-500 mb-4 drop-shadow-[0_0_20px_#10b981]">
            SYSTEM LIBERATED
          </h1>
          <p className="text-lg md:text-xl text-gray-400 mb-8">Vous êtes désormais un fantôme numérique.</p>
          <button 
            onClick={() => window.location.reload()}
            className="px-6 py-3 bg-transparent border border-emerald-500 text-emerald-500 hover:bg-emerald-500/10 transition-colors uppercase tracking-widest"
          >
            Reboot System
          </button>
        </div>
